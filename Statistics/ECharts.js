...
var MyChartDataFormate = {
    ...
    }  //需要分组的数据 is_stack(是否重叠 堆)
}
//图表所需要的Json模板格式生成
var MyChartOptionTempates = {
    ...
    Pie: function (data, name, formatter) {//data:数据格式：{name：xxx,value:xxx}...  
        ...
        return $.extend({}, MyEcharts.ChartOptionTemplates.CommonOption, option);

    },

    Lines: function (data, name, is_stack) { //data:数据格式：{name：xxx,group:xxx,value:xxx}...  

        var stackline_datas = MyEcharts.ChartDataFormate.FormateGroupData(data, 'line', is_stack);
        ...
        return $.extend({}, MyEcharts.ChartOptionTemplates.CommonLineOption, option);

    },

    Bars: function (data, name, is_stack) {//data:数据格式：{name：xxx,group:xxx,value:xxx}...  

        var bars_dates = MyEcharts.ChartDataFormate.FormateGroupData(data, 'bar', is_stack);
...
var MyEcharts = {
    ...
    ChartConfig: function (container, option) {
        // 路径配置  2.1.8版本之前需要配合zrender 第三方插件试用(....)
        //require.config({
        //    packages: [
        // {
        //     name: 'echarts',
        ...
        //    ]
        //});
        //  2.1.8版本之后 直接试用(需要echarts-optimizer生成dist)
        require.config({
            paths: {
                echarts: '../../Content/js/MyEcharts/dist',
            }
        })
    ...
    //数据格式化
    ChartDataFormate: MyChartDataFormate,
    //初始化图表类型
    ChartOptionTemplates: MyChartOptionTempates,
    //渲染图像
    Charts: MyCharts,
}
